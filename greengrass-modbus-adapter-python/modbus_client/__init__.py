...
# Installed from the futures backport for Python 2.7 support; `pip install futures`
...
def _client_worker_process(factory, input_queue, output_queue, is_shutdown):
    ...
    """
    logger.info("starting up worker : %s", threading.current_thread())
    client = factory()
    while not is_shutdown.is_set():
        try:
            workitem = input_queue.get(timeout=1)
            logger.debug("dequeue worker request: %s", workitem)
            if not workitem:
                continue
            try:
                logger.debug("executing request on thread: %s", workitem)
            ...
            logger.exception("Error while processing: %s", ex)
            
    logger.info("Request worker shutting down: %s", threading.current_thread())
...
def _manager_worker_process(output_queue, futures, is_shutdown):
    ...
    :param is_shutdown: Condition variable marking process shutdown
    """
    logger.info("starting up manager worker: %s", threading.current_thread())
    while not is_shutdown.is_set():
        try:
            workitem = output_queue.get()
            future = futures.get(workitem.work_id, None)
            logger.debug("dequeue manager response: %s", workitem)
            if not future:
                continue
            ...
            logger.debug("updated future result: %s", future)
            del futures[workitem.work_id]
        except Exception as ex:
            logger.exception("Error in manager:", ex)
    logger.info("manager worker shutting down: %s", threading.current_thread())
...
class ConcurrentClient(ModbusClientMixin):
    ...
    def shutdown(self):
        ...
        """
        logger.info("stating to shut down workers")
